...
describe('PostgresClient', () => {
	...
	describe('error handling edge cases', () => {
		...
		it('should handle rollback failure in transaction', async () => {
			...
			// Mock BEGIN to succeed, the transaction query to fail, and ROLLBACK to fail
			mockClient.query.mockImplementation(async (sql: string) => {
				...
				// Any other query is considered the failing one
				throw transactionError;
			});

			const callback = async (txClient: any) => {
				await txClient.query('SELECT * FROM table_that_fails');
			};
	...
	describe('transaction client wrapper', () => {
		...
		it('should handle transaction failure when COMMIT fails', async () => {
			...
			// Mock BEGIN to succeed, COMMIT to fail
			mockClient.query
				.mockResolvedValueOnce({ rows: [], rowCount: 0 }) // BEGIN
		...
		it('should handle transaction failure when ROLLBACK fails', async () => {
			...
			mockClient.query.mockImplementation(async (sql: string) => {
				...
				if (sql.includes('table_that_fails')) {
					throw queryError;
				}
				if (sql === 'ROLLBACK') {
					throw rollbackError;
		...
		it('should handle client release failure after a transaction error', async () => {
			const transactionError = new Error('Transaction failed');
			const releaseError = new Error('Release failed');
			...
			// Verify client release was attempted and its failure was logged
			expect(mockClient.release).toHaveBeenCalled();
			expect(mockLogger.error).toHaveBeenCalledWith('Failed to release PostgreSQL client', releaseError);
		});
	});
