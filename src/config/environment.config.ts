...
const EnvSchema = z.object({
  ...
  EXPIRATION_HOURS: z.string().transform(val => parseInt(val, 10)).pipe(z.number().min(1)).default('12'),
  
  // PostgreSQL Configuration
  PG_HOST: z.string().min(1, 'PG_HOST is required'),
  PG_PORT: z.string().transform(val => parseInt(val, 10)).pipe(z.number().min(1).max(65535)).default('5432'),
  ...
  PG_SSL: z.string().transform(val => val === 'true').pipe(z.boolean()).default('false'),
  PG_MAX_CONNECTIONS: z.string().transform(val => parseInt(val, 10)).pipe(z.number().min(1)).default('10'),
  PG_CONNECTION_TIMEOUT: z.string().transform(val => parseInt(val, 10)).pipe(z.number().min(1000)).default('30000'),
  PG_IDLE_TIMEOUT: z.string().transform(val => parseInt(val, 10)).pipe(z.number().min(1000)).default('30000'),
  
  // MongoDB Configuration
  MONGO_URI: z.string().min(1, 'MONGO_URI is required'),
  MONGO_DATABASE: z.string().min(1, 'MONGO_DATABASE is required'),
  MONGO_MAX_POOL_SIZE: z.string().transform(val => parseInt(val, 10)).pipe(z.number().min(1)).default('10'),
  MONGO_MIN_POOL_SIZE: z.string().transform(val => parseInt(val, 10)).pipe(z.number().min(0)).default('1'),
  MONGO_MAX_IDLE_TIME: z.string().transform(val => parseInt(val, 10)).pipe(z.number().min(1000)).default('30000'),
  MONGO_SERVER_SELECTION_TIMEOUT: z.string().transform(val => parseInt(val, 10)).pipe(z.number().min(1000)).default('30000'),
